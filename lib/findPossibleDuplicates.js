...
module.exports = function findPossibleDuplicates(groupedData) {
	...
	if (debug) console.log('similarities', JSON.stringify(similarities, null, '  '));
		...
		for (const key in group) {
			...
			while ((b = files.shift())) {
				...
				if (matchScore > 1) {
					...
					if (!possibleDuplicates[a][matchScore]) possibleDuplicates[a][matchScore] = [a]; // just to initiate property
	...
	if (debug) console.log('possibleDuplicates', JSON.stringify(possibleDuplicates, null, '  '));
