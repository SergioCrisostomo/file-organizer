...
async function processFolder(files, root, { verbose }) {
  ...
  const groupedData = files.map((file, i) =>
    groupData(file, stats[i], data[i], metaData[i])
  );
...
module.exports = async (
  dirs,
  { ext, recursive, verbose = true, process = true }
) => {
  const folders = await Promise.all(
    dirs.map((dir) => readFolder(dir, ext, recursive))
  );

  const duplicates = process
    ? await Promise.all(
        folders.map((files, i) => processFolder(files, dirs[i], { verbose }))
      )
    : null;

  return folders.reduce((obj, folder, i) => {
    obj[dirs[i]] = { files: folders[i] };
    if (duplicates) obj[dirs[i]].duplicates = duplicates[i];
    return obj;
  }, {});
};
