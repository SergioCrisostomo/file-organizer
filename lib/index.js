...
const getMemoryUsage = require("../server/utils/getMemoryUsage");
...
async function processFile(file, i, arr) {
  const verbose = false;
  ...
  if (verbose)
    console.log(
      `Processed file nr ${i} of ${
        arr.length
      }. Memory usage is ${getMemoryUsage()}Gb`
    );
  return groupedData;
}

async function analyseDuplicates(groupedData) {
  const verbose = false;
  const possibleDuplicates = await findPossibleDuplicates(groupedData);
...
module.exports = async (files) => {
  ...
  const report = await analyseDuplicates(groupedData);
